...
def agregar_pedido(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('agregar_pedido_producto')
    else:
        form = PedidoForm()
    return render(request, 'agregar_pedido.html', {'form': form})

def agregar_pedido_producto(request):
    if request.method == 'POST':
        pedido_id = request.POST.get('pedido_id')
        form = PedidoProductoForm(request.POST, pedido_id=pedido_id)
        if form.is_valid():
            form.save()
            return redirect('gestion_pedidos')        
    else:
        pedido_id = request.GET.get('pedido_id')  
        form = PedidoProductoForm(pedido_id=pedido_id)
    productos_disponibles = Producto.objects.all()
    return render(request, 'agregar_pedido_producto.html', {'productos_disponibles': productos_disponibles, 'form': form})
